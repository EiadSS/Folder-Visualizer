...
def test_tmt_init() -> None:
    eiad = TMTree("eiad", [])
    assert eiad.data_size == 0
    assert eiad._name is not None
    assert eiad.rect == (0, 0, 0, 0)
    assert eiad._subtrees == []
    assert eiad._expanded == False
    eiad = TMTree("eiad", [TMTree("lol", [TMTree("eid", [], 20)], 20),
                           TMTree('niz', [TMTree('bro', [], 10),
                                          TMTree('idk', [], 10)], 20),
                           TMTree('bruh', [], 10)], 50)
    assert eiad.data_size == 50
    assert eiad._subtrees[0]._parent_tree == eiad
    assert eiad._subtrees[0]._subtrees[0]._parent_tree == eiad._subtrees[0]
    assert eiad._subtrees[0]._expanded is True
    assert eiad._subtrees[0]._subtrees[0]._expanded is False


def test_File_init() -> None:
    eiad = FileSystemTree("/Users/Tala/Desktop/csc148/"
                          "/miscellaneous")
    print(eiad._name)
    assert eiad._name == 'miscellaneous'
    assert eiad._subtrees[0]._parent_tree == eiad
    assert eiad._subtrees[0]._subtrees[0]._parent_tree == eiad._subtrees[0]
    assert os.path.getsize("/Users/Tala/Desktop/csc148/miscellaneous"
                           "/.pytest_cache/.gitignore") == \
           eiad._subtrees[0]._subtrees[0].data_size
    print(eiad._subtrees[0]._name)
    bro = FileSystemTree(EXAMPLE_PATH)
    assert bro._name == "workshop"
    idk = FileSystemTree(os.getcwd())
    assert idk._expanded == True


def test_get_rectangles() -> None:
    eiad = FileSystemTree('/Users/Tala/Desktop/csc148/preps/prep3')
    eiad.update_rectangles((0, 0, 200, 100))
    brother = eiad.get_rectangles()
    eid = []
    for i in brother:
        eid.append(i[0])
        print(i[0])
    ...
    eiad = TMTree("eiad", [TMTree("lol", [TMTree("eid", [], 20)], 20),
                           TMTree('niz', [TMTree('bro', [], 10),
                                          TMTree('idk', [], 10)], 20),
                           TMTree('bruh', [], 10)], 50)
    assert eiad.update_data_sizes() == 50
    eiad.change_size(3)
    assert eiad.update_data_sizes() == 200
    eiad.change_size(-.75)
    assert eiad.update_data_sizes() == 50
    assert eiad._subtrees[1]._subtrees[1].data_size == 10
    eiad.change_size(.6)
    assert eiad._subtrees[1]._subtrees[1].data_size == 16
    eiad.change_size(.6)
    assert eiad._subtrees[1]._subtrees[1].data_size == 26
...
def test_task3() -> None:
    ...
    for i in brother:
        eid.append(i[0])
        print(i[0])
    assert eiad.get_tree_at_position((43, 27)).rect == (0, 0, 80, 100)
    assert eiad.get_tree_at_position(((500, 100))) is None
...
def test_move() -> None:
    eiad = TMTree("eiad", [TMTree("lol", [TMTree("eid", [], 20)], 20),
                           TMTree('niz', [TMTree('bro', [], 10),
                                          TMTree('idk', [], 10)], 20),
                           TMTree('bruh', [], 10)], 50)
    eiad._subtrees[2].move(eiad._subtrees[0])
    assert eiad._subtrees[0]._subtrees[1]._name == 'bruh'
    assert len(eiad._subtrees) == 2
    assert eiad._subtrees[0]._subtrees[1]._parent_tree == eiad._subtrees[0]
    assert len(eiad._subtrees[0]._subtrees) == 2
    assert eiad._subtrees[0]._subtrees[1].delete_self() is True
    assert len(eiad._subtrees[0]._subtrees) == 1
    assert eiad.delete_self() is False
