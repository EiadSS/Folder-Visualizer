...
class PaperTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: List[TMTree], doc: dict = {},
                 authors: str = '',
        ...
        if not all_papers:
            ...
            if by_year:
                TMTree.__init__(self, name, subtrees, citations)
                if self.doc == {}:
                    self.doc = _load_papers_to_dict(by_year)
                self.name = name
                self._subtrees = []
                for year in self.doc:
                    if year is not "info":
                        x = self.doc[year]
                        self._subtrees.append(
                            PaperTree(year, [], x, all_papers=True,
                                      by_year=True))
                    else:
                        self._subtrees.append(year, [], doc, )
